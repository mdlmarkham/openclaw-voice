...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def process_buffer():
        """Transcribe buffered audio, stream AI response, and send TTS audio."""
        nonlocal audio_buffer, is_listening, session_agent
        is_listening = False
        ...
        if audio_buffer:
            try:
                ...
                if transcript.strip():
                        ...
                        while any(sep in sentence_buffer for sep in SENTENCE_SEPS):
                            ...
                            if earliest_idx < len(sentence_buffer):
                                ...
                                if sentence:
                                    ...
                                    audio_seq = await speak_sentence(websocket, sentence, audio_seq)
                                    ...
                                    if t_first_audio is None:
                                        ...
                                        logger.info(
                                            ...
                                            f"(STT={t_stt:.2f}s, first_token={t_first_token:.2f}s, "
                                            f"TTS_1st={tts_time:.2f}s)"
                                        )
                            else:
                                break
                    ...
                    logger.info(
                        f"Response complete: {t_total:.2f}s total, "
                        f"{full_response[:80]}..."
                    )
            except Exception as inner_err:
                logger.error(f"Processing error: {inner_err}")
                await websocket.send_json({"type": "error", "message": str(inner_err)})
            ...
            if msg_type == "start_listening":
                ...
                try:
                    ...
                    if total_samples > MAX_AUDIO_BUFFER_SAMPLES:
                        logger.warning(
                            f"Audio buffer cap reached ({total_samples} samples), processing"
                        )
